...
def create_order(request):
    ...
    try:
        ...
        error_type = type(e).__name__
        error_msg = str(e)
        # Ensure amount_paise is defined for the logger
        amount_val = amount_paise if 'amount_paise' in locals() else 'unknown'
        logger.exception(
            f"create_order: Razorpay Order.create FAILED — "
            f"user={request.user.email}, plan={plan}, amount={amount_val}, "
            f"error_type={error_type}, error_msg={error_msg}"
        )
...
def verify_payment(request):
    ...
    try:
        ...
        logger.exception(f"verify_payment: Failed to construct success response: {e}")
        return JsonResponse({
            'error': 'Payment verified but failed to load plan details. Please refresh.',
            'verified': True,
            'success': True,
            'plan': plan,
            'tier': 'premium',
